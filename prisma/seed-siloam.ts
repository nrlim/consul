...
// ----------------------------------------------------------------
function extractTiers(item: Record<string, unknown>, columns?: string[]): Record<string, number> {
  ...
  for (const [k, v] of Object.entries(item)) {
    ...
    if (price !== null) {
      let tierName = k;
      if (k.startsWith("price_") && columns) {
        const colIdx = parseInt(k.replace("price_", ""), 10) - 1;
        if (columns[colIdx]) {
          tierName = columns[colIdx].toLowerCase();
        }
      }
      tiers[tierName] = price;
    }
  }
  return tiers;
}
...
function buildEntries(
  ...
  for (const item of items) {
    ...
    const tiers = extractTiers(item, columns);
    ...
    let rawCode = (item.code as string) || (item.service_code as string) || (item.item_code as string);
    let procedureCode = rawCode 
      ? cleanString(String(rawCode)).replace(/\s+/g, '-')
...
async function main() {
  ...
  // ── 3. Parse JSON & Build Entries ──────────────────────────────
  const rawDataStr = fs.readFileSync(path.join(__dirname, "../sample-data/master-data-docs/buku-tarif-siloam.json"), "utf8");
  const rawData = JSON.parse(rawDataStr);

  const globalIndex = { count: 0 };
  const allEntries: Array<Record<string, unknown>> = [];
  const columns = Array.isArray(rawData.columns) ? rawData.columns as string[] : undefined;
  let sectionCount = 0;
  ...
  for (const [key, value] of Object.entries(rawData)) {
    ...
    if (Array.isArray(value)) {
      const entries = buildEntries(provider.id, key, value as Record<string, unknown>[], globalIndex, columns);
      console.log(`  📂 ${key}: ${entries.length} / ${value.length} entries`);
