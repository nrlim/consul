...
You are a senior audit klaim reviewer for healthcare claims across multiple clients and payer/provider configurations. Your task is to perform a clinically sound fallback review when local diagnosis-procedure mapping is absent or incomplete.
...
8. Prefer HIGH confidence only for exact or near-exact product/ingredient + strength/form matches.`,
  ...
  async generateClinicalPathway(diagnosisCode: string, diagnosisName: string, diagnosisContext?: Array<{ code: string; name?: string; type?: string; sequence?: number }>): Promise<{ data: any; usage?: Usage }> {
    ...
    // audit klaim output is a nested multi-phase object. Keep the request
    ...
    const { object, usage } = await generateObject({
      ...
      schemaDescription: 'A audit klaim object. Return raw JSON only. Do not wrap in markdown fences.',
      maxOutputTokens,
      timeout,
      experimental_repairText: async ({ text }) => repairClinicalPathwayJsonText(text),
      prompt: `Generate a realistic audit klaim for ${diagnosisCode} - ${diagnosisName} suitable for Indonesian healthcare context.
...
- Provide a brief _mappingNotes explaining your interpretation decisions`;
  ...
  async estimateDiagnosisLos(diagnosisCode: string, diagnosisName: string, thresholds?: { overstayDays?: number; understayDays?: number }): Promise<{ data: any; usage?: Usage }> {
    const schema = z.object({
      ...
      }).describe('Recommended therapy pathway that justifies the LOS estimate. This feeds into audit klaim generation.'),
    ...
    const { object, usage } = await generateObject({
      ...
      system: `You are a senior audit klaim analyst and medical reviewer for multi-client healthcare claim review. Your task is to provide LOS estimates that are:
