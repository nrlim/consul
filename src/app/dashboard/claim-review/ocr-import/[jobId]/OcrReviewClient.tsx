...
export default function OcrReviewClient({ jobId }: OcrReviewClientProps) {
  ...
  const handleForwarded = (claimJobId?: string) => {
    ...
    if (claimJobId) {
      window.location.href = `/dashboard/claim-review/${claimJobId}`;
    } else {
      window.location.href = "/dashboard/claim-review/ocr-import";
    }
  };
  ...
  if (error || !scoringResult) {
    return (
      <div className="space-y-4">
        <Link href="/dashboard/claim-review/ocr-import" className="inline-flex items-center text-sm font-medium text-sky-700 hover:underline">
    ...
    <div className="space-y-6">
      ...
      <OcrReviewTable 
        ...
        onForward={(claimJobId) => {
          if (claimJobId) {
            router.push(`/dashboard/claim-review/${claimJobId}`);
          } else {
            router.push('/dashboard/claim-review');
          }
        }}
      />
    </div>
