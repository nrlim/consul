          ...
          <tbody className="divide-y divide-border">
            {paginatedJobs.length === 0 ? <tr><td colSpan={7} className="px-5 py-12 text-center text-sm font-light text-muted-foreground">Tidak ada validasi yang sesuai filter.</td></tr> : paginatedJobs.map((job) => {
              ...
              return <tr key={job.id} className="transition-colors hover:bg-muted/50">
                ...
                <td className="px-5 py-4 text-right"><Link href={`/dashboard/claim-review/${job.id}`} className="inline-flex items-center justify-center rounded-md border border-border px-3 py-1.5 text-[11px] font-mono uppercase tracking-[0.2em] text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">View Results</Link></td>
              </tr>;
            })}
          </tbody>
        </table>
