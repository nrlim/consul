  ...
  try {
    ...
    if (ocrJob.status === "FORWARDED" && ocrJob.claimJobId) {
      return NextResponse.json({
        ...
        redirectUrl: `/dashboard/claim-review/${ocrJob.claimJobId}`,
    ...
    if (!hasUnlimitedQuota) {
      const requestDebit = await debitClientRequestUsage({
        clientId: resolvedClientId,
        jobId: claimJob.id,
        description: "Audit Klaim validation request from SnapText OCR invoice",
      });
    ...
    return NextResponse.json({
      ...
      redirectUrl: `/dashboard/claim-review/${claimJob.id}`,
